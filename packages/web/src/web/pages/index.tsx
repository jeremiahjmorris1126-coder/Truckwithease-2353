    ...
    <div className="min-h-screen bg-[#0a0a0a] text-[#F5F5F5]">
      ...
      <header className="sticky top-0 z-30 twe-navy-grad text-white">
        <div className="max-w-6xl mx-auto flex items-center justify-between px-6 h-16">
          <div className="flex items-center gap-2">
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#C9A84C]"><Truck className="h-5 w-5 text-[#C9A84C]" /></div>
            <span className="text-lg font-bold">Truck<span className="text-[#C9A84C]">WithEase</span></span>
          </div>
          <nav className="hidden md:flex items-center gap-6 text-sm text-[#C9C9C9]">
            <a href="#features" className="hover:text-white">Features</a>
            <a href="#why" className="hover:text-white">Why us</a>
            <Link to="/app/pricing" className="hover:text-white">Pricing</Link>
          </nav>
          <Link to="/app" className="rounded-lg bg-[#C9A84C] px-4 py-2 text-sm font-semibold text-[#F5F5F5] hover:bg-[#FFD700]">Open App</Link>
        </div>
      </header>
      ...
      <section className="twe-navy-grad text-white">
        <div className="max-w-6xl mx-auto px-6 pt-16 pb-24 text-center">
          ...
          <p className="mt-5 text-lg text-[#C9C9C9] max-w-2xl mx-auto">
      ...
      <section id="features" className="max-w-6xl mx-auto px-6 py-20">
        <div className="text-center mb-12">
          ...
          <p className="mt-3 text-[#8A8A8A] max-w-2xl mx-auto">One subscription replaces a glovebox full of apps and devices.</p>
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {FEATURES.map((f) => {
            const Icon = f.icon;
            return (
              <div key={f.title} className="rounded-xl bg-[#161616] border border-[#222222] p-6 hover:border-[#C9A84C] hover:shadow-md transition-all">
                <div className="flex h-11 w-11 items-center justify-center rounded-lg bg-[#C9A84C]"><Icon className="h-5 w-5 text-[#C9A84C]" /></div>
                <h3 className="mt-4 font-bold">{f.title}</h3>
                <p className="mt-1.5 text-sm text-[#8A8A8A]">{f.desc}</p>
              </div>
            );
          })}
        </div>
      ...
      {/* Why */}
      <section id="why" className="bg-[#161616] border-y border-[#222222]">
              ...
              {[
                ...
                return (
                  <div key={r.t} className="flex gap-4">
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-[#C9A84C]/15"><Icon className="h-5 w-5 text-[#FFD700]" /></div>
                    <div><div className="font-semibold">{r.t}</div><p className="text-sm text-[#8A8A8A] mt-0.5">{r.d}</p></div>
                  </div>
                );
              })}
            </div>
      ...
      <section className="max-w-6xl mx-auto px-6 py-20 text-center">
        ...
        <p className="mt-3 text-[#8A8A8A]">Open the full demo — no login, no card, full access.</p>
        <Link to="/app" className="mt-6 inline-flex items-center gap-2 rounded-lg bg-[#C9A84C] px-6 py-3 font-semibold text-[#0a0a0a] hover:bg-[#FFD700]">Open TruckWithEase <ArrowRight className="h-4 w-4" /></Link>
      </section>

      <footer className="border-t border-[#222222] py-8 text-center text-sm text-[#8A8A8A]">
        <div className="flex items-center justify-center gap-2 mb-2">
          <div className="flex h-6 w-6 items-center justify-center rounded bg-[#C9A84C]"><Truck className="h-3.5 w-3.5 text-[#C9A84C]" /></div>
          <span className="font-bold text-[#F5F5F5]">Truck<span className="text-[#FFD700]">WithEase</span></span>
        </div>
