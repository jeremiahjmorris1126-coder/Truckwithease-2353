...
export default function MapPage() {
  ...
  if (positions.isError) return <Card className="p-5 border-[#ef4444]"><p className="text-sm text-[#ef4444]">Fleet positions could not be loaded. Refresh to retry.</p></Card>;
  ...
  return (
    <div>
      ...
      {!HAS_MAPS && (
        <div className=" bg-[#d4af37]/10 border border-[#d4af37]/30 px-4 py-2.5 text-sm text-[#8B6914] mb-4">
          ...
          <div className="relative aspect-[16/10] twe-navy-grad">
            ...
            <svg className="absolute inset-0 w-full h-full opacity-20" preserveAspectRatio="none">
              {Array.from({ length: 12 }).map((_, i) => (
                <line key={`h${i}`} x1="0" y1={`${(i / 12) * 100}%`} x2="100%" y2={`${(i / 12) * 100}%`} stroke="#94a3b8" strokeWidth="0.5" />
              ))}
              {Array.from({ length: 16 }).map((_, i) => (
                <line key={`v${i}`} x1={`${(i / 16) * 100}%`} y1="0" x2={`${(i / 16) * 100}%`} y2="100%" stroke="#94a3b8" strokeWidth="0.5" />
              ))}
            </svg>
            {ps.map((p) => {
        ...
        <div className="space-y-3">
          {sel ? (
            <Card className="p-5" accent>
              <div className="flex items-center gap-2 mb-3"><Truck className="h-5 w-5 text-[#d4af37]" /><b>{sel.truckNumber}</b><Badge status={sel.status} /></div>
              <div className="text-sm space-y-2">
                <div className="flex justify-between"><span className="text-[#94a3b8]">Driver</span><span className="font-medium">{sel.name}</span></div>
                <div className="flex justify-between"><span className="text-[#94a3b8]">Speed</span><span className="font-mono-data">{Math.round(sel.speed ?? 0)} mph</span></div>
                <div className="flex justify-between"><span className="text-[#94a3b8]">Position</span><span className="font-mono-data text-xs">{sel.lat?.toFixed(3)}, {sel.lng?.toFixed(3)}</span></div>
              </div>
              <a href={`https://www.google.com/maps?q=${sel.lat},${sel.lng}`} target="_blank" rel="noreferrer" className="mt-4 flex items-center justify-center gap-2  bg-[#d4af37] text-[#08090c] py-2 text-sm font-medium">
                <Navigation className="h-4 w-4" />Open in Maps
              </a>
            </Card>
          ) : (
            <Card className="p-5 text-center text-sm text-[#94a3b8]"><MapPin className="h-6 w-6 mx-auto mb-2 text-[#d4af37]" />Tap a truck to see details</Card>
          )}
          <Card className="p-4">
            <div className="text-xs font-semibold uppercase text-[#94a3b8] mb-2">Fleet ({ps.length})</div>
            <div className="space-y-1">
              {ps.map((p) => (
                <button key={p.id} onClick={() => setSelected(p.id)} className={`w-full flex items-center justify-between  px-2.5 py-2 text-sm ${p.id === selected ? "bg-[#d4af37]/10" : "hover:bg-[#08090c]"}`}>
