...
export const fleetAssets = sqliteTable("fleet_assets", {
  id: text("id").primaryKey(),
  name: text("name").notNull(),
  assetType: text("asset_type").notNull(), // truck, trailer, equipment, attachment, other
  status: text("status").notNull().default("available"), // active, available, maintenance, retired
  unit: text("unit"),
  vin: text("vin"),
  ...
  notes: text("notes"),
  nextServiceAt: integer("next_service_at", { mode: "timestamp" }),
  createdAt: integer("created_at", { mode: "timestamp" }).notNull().$defaultFn(() => new Date()),
...
/**
 * haptic_playbacks — one append-only row per vibration pattern actually played
 * on a real device.
 *
 * WHY: the Haptic page shipped a client-side library that documented its
 * patterns as [duration_ms, pause_ms, repeat_count] and then handed those exact
 * arrays to navigator.vibrate(), which reads them as ALTERNATING on/off
 * milliseconds. 'DANGER': [500, 200, 3] therefore vibrated 500ms, paused 200ms,
 * then vibrated for 3 MILLISECONDS — not three long pulses. Every labelled
 * pattern was wrong the same way, and nothing anywhere recorded whether a
 * pattern ever reached a device at all.
 *
 * userId is nullable on purpose: an anonymous visitor testing a vibration on a
 * phone is still a real, useful measurement. (Unlike an agreement acceptance,
 * which is worthless if it cannot be attributed.)
 *
 * deviceSupported is what the browser itself reported ('vibrate' in navigator),
...
export const hapticPlaybacks = sqliteTable("haptic_playbacks", {
  ...
  patternVersion: text("pattern_version").notNull(), // sha256 of the canonical on/off encoding served
